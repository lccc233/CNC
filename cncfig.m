...
jcqws=str2num(get(handles.edit17,'string'));
n=2^jcqws;
jvx=abs(cstp_y-cip_y);
jvy=abs(cstp_x-cip_x);
loaderr=get(handles.loader,'value')
jrx=0;
jry=0;
if loaderr == 2
    jrx=2^(jcqws-1);
...
% --- Executes on selection change in loader.
function loader_Callback(hObject, eventdata, handles)
% hObject    handle to loader (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns loader contents as cell array
%        contents{get(hObject,'Value')} returns selected item from loader


% --- Executes during object creation, after setting all properties.
function loader_CreateFcn(hObject, eventdata, handles)
% hObject    handle to loader (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
